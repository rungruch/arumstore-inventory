                    ...
                    {productTransactions.map((transaction, index) => {
                      ...
                      return (
                        <tr key={transaction.id || index} className="hover:bg-gray-50 dark:hover:bg-zinc-700">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                              ${transaction.status === 'COMPLETED' || transaction.status === OrderStatus.APPROVED ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' :
                                transaction.status === OrderStatus.PENDING ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400' :
                                transaction.status === OrderStatus.SHIPPING ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400' :
