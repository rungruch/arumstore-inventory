  ...
  return (
    ...
    <div className="container mx-auto p-3 sm:p-5 min-h-screen bg-gray-50 dark:bg-zinc-900">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">หมวดหมู่</h1>
          <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300">
            จำนวน {totalCategories} รายการ
          </h2>
            ...
            <FlexTable
              ...
              customHeader={
                <tr className="text-left h-[9vh] bg-gradient-to-r from-gray-50 to-gray-100 dark:from-zinc-700 dark:to-zinc-600 border-b border-gray-200 dark:border-zinc-600">
                  ...
                  <th className="p-4 w-[35%] font-bold text-gray-900 dark:text-gray-100 tracking-wide">ชื่อหมวดหมู่</th>
                  <th className="p-4 font-bold text-gray-900 dark:text-gray-100 tracking-wide">จำนวนสินค้า</th>
                  <th className="p-4 font-bold text-gray-900 dark:text-gray-100 tracking-wide">มูลค่าสินค้าคงเหลือ</th>
                  <th className="p-4 font-bold text-gray-900 dark:text-gray-100 tracking-wide">มูลค่าสินค้ารอยืนยัน</th>
                  <th className="p-4 w-[5%] font-bold text-gray-900 dark:text-gray-100"> </th>
                </tr>
              }
              customRow={(category, index) => (
                <tr key={category.id} className="border-b border-gray-100 dark:border-zinc-600 transition-all duration-300 ease-in-out hover:bg-gradient-to-r hover:from-gray-50 hover:to-gray-100 dark:hover:from-zinc-700 dark:hover:to-zinc-600 group">
                  <td className="p-4 w-[5%] text-center text-gray-700 dark:text-gray-300 font-medium">{index + 1 + (currentPage - 1) * pageSize}</td>
                  <td className="p-4 w-[35%] text-gray-700 dark:text-gray-300">
                    <Link
                  ...
                  <td className="p-4 text-gray-700 dark:text-gray-300 font-medium">{skuCount?.skus?.[category.category_name]?.count || 0}</td>
                  <td className="p-4 font-mono text-gray-900 dark:text-gray-100 font-semibold">฿{skuCount?.skus?.[category.category_name]?.totalIncome?.toLocaleString() || 0}</td>
                  <td className="p-4 font-mono text-gray-900 dark:text-gray-100 font-semibold">฿{skuCount?.skus?.[category.category_name]?.totalPendingIncome?.toLocaleString() || 0}</td>
