...
const styles = StyleSheet.create({
  ...
  tableCol2: { width: '10%', textAlign: 'center', borderRight: '1px solid #e0e0e0' },
  tableCol3: { width: '15%', borderRight: '1px solid #e0e0e0'},
  tableCol4: { width: '25%', borderRight: '1px solid #e0e0e0' },
  tableCol5: { width: '15%', textAlign: 'center',  borderRight: '1px solid #e0e0e0' },
  tableCol6: { width: '15%', textAlign: 'right', borderRight: '1px solid #e0e0e0' },
  tableCol7: { width: '15%', textAlign: 'right' },
  tableCol1_data: { width: '5%', textAlign: 'center', borderRight: '1px solid #e0e0e0' },
  tableCol2_data: { width: '10%', textAlign: 'center', borderRight: '1px solid #e0e0e0', padding: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  tableCol3_data: { width: '15%', textAlign: 'center', borderRight: '1px solid #e0e0e0',},
  tableCol4_data: { width: '25%', borderRight: '1px solid #e0e0e0'},
  tableCol5_data: { width: '15%', textAlign: 'center',  borderRight: '1px solid #e0e0e0'},
  tableCol6_data: { width: '15%', textAlign: 'right', borderRight: '1px solid #e0e0e0'},
  tableCol7_data: { width: '15%', textAlign: 'right' },
  totalItemsRow: {
    flexDirection: 'row',
    padding: 5,
    borderBottom: '1px solid #e0e0e0',
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.table}>
        ...
        <View style={styles.tableHeader}>
          ...
          <Text style={styles.tableCol2}>รูป</Text>
          <Text style={styles.tableCol3}>รหัสสินค้า</Text>
          <Text style={styles.tableCol4}>ชื่อสินค้า</Text>
          <Text style={styles.tableCol5}>จำนวน</Text>
          {(data.orderInfo.titleDocument !== 'ใบส่งสินค้า') && (
          <>
            <Text style={styles.tableCol6}>ราคา/หน่วย</Text>
            <Text style={styles.tableCol6}>ส่วนลด/หน่วย</Text>
            <Text style={styles.tableCol7}>จำนวนเงิน</Text>
          </>
          )}
          {(data.orderInfo.titleDocument === 'ใบส่งสินค้า') && (
          <>
            <Text style={styles.tableCol7}>จำนวนได้รับ</Text>
          </>
          )}
        </View>
        ...
        {data.items.map((item : any, index : any) => (
          <View key={index} style={styles.tableRow}>
            ...
            <View style={styles.tableCol2_data}>
            ...
            <Text style={styles.tableCol3_data}>{item.id}</Text>
            <Text style={styles.tableCol4_data}>  {item.name}</Text>
            <Text style={styles.tableCol5_data}>{item.quantity} {item.unitType}</Text>
            {(data.orderInfo.titleDocument !== 'ใบส่งสินค้า') && (
            <>
            <Text style={styles.tableCol6_data}>{item.unitPrice.toLocaleString()}  </Text>
            <Text style={styles.tableCol6_data}>{item.discount.toLocaleString()}  </Text>
            <Text style={styles.tableCol7_data}>{item.total.toLocaleString()}</Text>
            </>
            )}
            {(data.orderInfo.titleDocument === 'ใบส่งสินค้า') && (
            <>
      ...
      {(data.orderInfo.showPriceSummary === true) && (
        ...
        {(data.paymentSummary.paymentSummaryEnabled === false) && (
          <Text style={{ width: '70%', textAlign: 'left', borderRight: '1px solid #e0e0e0'}}>  ชำระเงินด้วย {data.orderInfo.paymentMethod}</Text>
        )}
